...
export class ItemMasterSearchFieldIndexes1787300000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_item_master_row_alias_trgm" ON "item_master_row" USING gin ("alias" gin_trgm_ops);`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_item_master_row_sap_item_code_trgm" ON "item_master_row" USING gin ("sap_item_code" gin_trgm_ops);`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_item_master_row_hsn_description_trgm" ON "item_master_row" USING gin ("hsn_description" gin_trgm_ops);`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX "IDX_item_master_row_hsn_description_trgm";`,
    );
    await queryRunner.query(
      `DROP INDEX "IDX_item_master_row_sap_item_code_trgm";`,
    );
