...
export class SearchIndexFixed1787120512574 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX idx_voucher_s9_vch ON voucher (company_id, valid_to, vch_no_norm varchar_pattern_ops);`,
    );
    await queryRunner.query(
      `CREATE INDEX idx_voucher_s9_amt ON voucher (company_id, valid_to, total_amount);`,
    );
    await queryRunner.query(
      `CREATE INDEX idx_voucher_s9_pty ON voucher (company_id, valid_to, party_name varchar_pattern_ops);`,
    );
  }
