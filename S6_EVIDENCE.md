...
| 15 | Anonymous `POST /api/search` and `GET /api/vouchers/1` are 401. Finance cannot publish (403) | Passed in e2e (`finance cannot publish`, `search and get voucher without token is 401`) |
| 16 | `audit_event` has `publish`, `unpublish` (hold), `search`, `voucher_open` after the e2e | Passed in e2e (`search and voucher_open and publish and unpublish are audited`) |
